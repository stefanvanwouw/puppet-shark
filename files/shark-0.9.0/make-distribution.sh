...
if [ "$MAKE_TGZ" == "true" ]; then
	echo "Making shark-$VERSION-hadoop_$SPARK_HADOOP_VERSION-bin.tar.gz"
else
...
if [ "$MAKE_TGZ" == "true" ]; then
  TARDIR="$FWDIR/shark-$VERSION"
  cp -r "$DISTDIR" "$TARDIR"
  tar -zcf "shark-$VERSION-hadoop_$SPARK_HADOOP_VERSION-bin.tar.gz" -C "$FWDIR" "shark-$VERSION"
  rm -rf "$TARDIR"
fi
